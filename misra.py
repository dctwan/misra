...
# python misra.py --rule-texts=<pth-to-rule-texts> main.cpp.dump
#
...
class MisraChecker:
    ...
    def misra_30_3(self, data):
        for token in data.tokenlist:
            if token.str == 'sprintf' and isFunctionCall(token.next): 
                name, args = cppcheckdata.get_function_call_name_args(token)  
                bufferToken = args[0]
                formatToken = args[1]
                bufferNameToken = bufferToken.variable.nameToken
                bufferSize = int(bufferNameToken.next.next.str)

                format_size_str = re.search('%.[0-9]*s', formatToken.str).group()
                st = 1
                end = 0
                for index in range(len(format_size_str)):
                    if format_size_str[index] == 's':
                        end = index
                format_size = 0
                if (end - st) > 1:
    ...
    def parseDump(self, dumpfile):
        ...
        for cfgNumber, cfg in enumerate(data.iterconfigurations()):
            ...
            if cfgNumber == 0:
                ...
                self.executeCheck(3001, self.misra_30_1, data.rawTokens)
